...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld)
        payable
        ERC721("Brise Naming Service", "BNS")
    {
    ...
    function register(string calldata name) public payable {
        ...
        require(msg.value >= _price, "Not enough Brise paid");
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Brise name service", "image": "data:image/svg+xml;base64,',
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        uint256 return_price;
        require(len > 0);

        //array stuff
        string[3] memory names = ["london", "web3", "brise"];
        for (uint i=0; i<names.length; i++) {
            
            if (keccak256(abi.encodePacked(name)) == keccak256(abi.encodePacked(names[i]))){
                uint256 return_price = 9 * 10**1;
            }
            else if (len == 3) {
                uint256 return_price = 5 * 10**1; // 5 Brise = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Brise cause the faucets don't give a lot
            }
            else if (len == 4) {
                uint256 return_price = 3 * 10**1; // To charge smaller amounts, reduce the decimals. This is 0.3
            }
            else {
                uint256 return_price = 1 * 1;
            }
        }
    ...
    function withdraw() public onlyOwner {
        ...
        require(success, "Failed to withdraw Brise");
    }
}